...
def is_migraine_related(row: dict[str, Any]) -> bool:
...
def main() -> None:
    ...
    migraine_rows = [r for r in rows if is_migraine_related(r)]
    examples = []
    ...
    train_path = os.path.join(out_dir, "migraine_train.jsonl")
    val_path = os.path.join(out_dir, "migraine_val.jsonl")
    test_path = os.path.join(out_dir, "migraine_test.jsonl")

    write_jsonl(train_path, train_rows)
    write_jsonl(val_path, val_rows)
    write_jsonl(test_path, test_rows)

    print(f"Total migraine candidates: {len(migraine_rows)}")
