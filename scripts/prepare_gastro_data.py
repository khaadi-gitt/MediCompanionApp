...
def is_gastro_related(row: dict[str, Any]) -> bool:
...
def main() -> None:
    ...
    gastro_rows = [r for r in rows if is_gastro_related(r)]
    examples = []
    for row in gastro_rows:
    ...
    train_path = os.path.join(out_dir, "gastro_train.jsonl")
    val_path = os.path.join(out_dir, "gastro_val.jsonl")
    test_path = os.path.join(out_dir, "gastro_test.jsonl")

    write_jsonl(train_path, train_rows)
    write_jsonl(val_path, val_rows)
    write_jsonl(test_path, test_rows)

    print(f"Total gastro candidates: {len(gastro_rows)}")
